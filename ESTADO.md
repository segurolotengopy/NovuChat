...
- **Construidos, revisados por `seguridad` y corregidos**, cada uno en su rama local sin
  subir: **A-0** `prepago/modulo-y-cortes` (módulo puro, gracia 48 h, calendario
  D-5/D-1/D0/D+2/D+4, corte en **modo observación**), **A-1** `prepago/pagos-y-carga-manual`
  (pagos con TCO, carga manual con evidencia), **A-2** `prepago/cliente-cobrador` (cliente
  del contrato real del cobrador, contra un doble) y **B-1** `flujos/ensamblador` (el código
  del vertical de reservas sale del JSON; los ocho JSON quedan byte a byte iguales).
  **A-5** `prepago/plantillas-cobranza`: ocho plantillas de utilidad redactadas, sin
  presentar a Meta.
- **El contrato del cobrador ya existe** en el proyecto de cobros (bloques 0 y 1 fusionados
  el 20/09; el aviso firmado en curso). El prompt de C y `Analisis/36` decían lo contrario:
  quedaron corregidos. Diseño en `admin/DISENO.md` §4undecies.
- **Riesgo para el demo:** siete commits de flujos (#119 a #127) fusionados en `main` después
  de `v0.6.0` no tienen registro de publicación en n8n. No se tocó nada; se publica tras el demo.
- **Compuerta del demo vigente:** nada desplegado, etiquetado, publicado ni presentado a Meta.
