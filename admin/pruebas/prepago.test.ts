...
import {
  ...
  fechaFinDelPeriodo, finDelPeriodoMs, montoDe, periodoAnterior, periodoDe,
...
describe('Los planes son los de la presentación comercial', () => {
  it('tres planes, con las cifras de la diapositiva', () => {
    expect(PLANES.base).toMatchObject({ mensualidad: 250, conversaciones: 300 });
    expect(PLANES.crecimiento).toMatchObject({ mensualidad: 450, conversaciones: 1000 });
    expect(PLANES.corporativo).toMatchObject({ mensualidad: 850, conversaciones: 2500 });
    expect(BOLSA).toEqual({ conversaciones: 150, precio: 50 });
    expect(PRUEBA.conversaciones).toBe(20);
  });

  it('el monto de un pago sale de la tabla, nunca del cliente', () => {
    expect(montoDe({ tipo: 'mensualidad', plan: 'crecimiento', meses: 1 })).toBe(450);
    expect(montoDe({ tipo: 'mensualidad', plan: 'base', meses: 3 })).toBe(750);
    expect(montoDe({ tipo: 'bolsa', cantidad: 2 })).toBe(100);
    // Cero o negativo se toma como uno: nunca un pago gratis.
    expect(montoDe({ tipo: 'bolsa', cantidad: 0 })).toBe(50);
  });
});
...
describe('Estado del servicio · prepago', () => {
  ...
  it('con el mes pagado y conversaciones, opera', () => {
    ...
    expect(e).toMatchObject({ operativo: true, motivo: null, cubierto: true, incluidas: 300,
      consumidas: 10, restanteDelPlan: 290, disponibles: 290, mensualidad: 250 });
  });
  ...
  it('al agotar las incluidas se corta, salvo que haya bolsa', () => {
    expect(estadoDeServicio(pagado, 300, '2026-09'))
      .toMatchObject({ operativo: false, motivo: 'sin_conversaciones', disponibles: 0 });
    expect(estadoDeServicio({ ...pagado, bolsa: 150 }, 300, '2026-09'))
      .toMatchObject({ operativo: true, restanteDelPlan: 0, disponibles: 150 });
    expect(estadoDeServicio({ ...pagado, bolsa: 150 }, 450, '2026-09'))
      .toMatchObject({ operativo: true, disponibles: 150 });
  });
  ...
  it('las bolsas NO sostienen el servicio si el mes no está pagado', () => {
    ...
    const e = estadoDeServicio({ ...pagado, bolsa: 300 }, 0, '2026-10');
    expect(e.operativo).toBe(false);
    expect(e.motivo).toBe('sin_pago');
    expect(e.bolsa).toBe(300);
  });
  ...
  it('un plan desconocido cae al plan base, nunca a ilimitado', () => {
    ...
    expect(e.incluidas).toBe(300);
  });

  it('valores basura en los saldos cuentan como cero', () => {
    const e = estadoDeServicio({ ...pagado, bolsa: 'muchas', bolsaPrueba: -5 }, 300, '2026-09');
...
describe('Estado del servicio · mes de prueba', () => {
  ...
  it('terminado el mes de prueba sin elegir plan, se corta por falta de pago', () => {
    ...
    expect(e).toMatchObject({ operativo: false, motivo: 'sin_pago', enPrueba: false, mensualidad: 0 });
  });
...
describe('Consumo de una conversación nueva', () => {
  ...
  it('mientras el plan tiene conversaciones, no toca ninguna bolsa', () => {
    const c = consumoDeConversacion(estadoDeServicio({ ...pagado, bolsa: 150 }, 10, '2026-09'));
    expect(c).toEqual({ campoBolsa: null, cortaDespues: false });
  });

  it('agotado el plan, descuenta de la bolsa comprada', () => {
    const c = consumoDeConversacion(estadoDeServicio({ ...pagado, bolsa: 150 }, 300, '2026-09'));
    expect(c).toEqual({ campoBolsa: 'bolsa', cortaDespues: false });
  });

  it('la última disponible avisa que después de ella se corta', () => {
    expect(consumoDeConversacion(estadoDeServicio({ ...pagado, bolsa: 1 }, 300, '2026-09')))
  ...
  it('en el mes de prueba descuenta de la bolsa de prueba antes que de la comprada', () => {
    const e = estadoDeServicio({ modalidad: 'prueba', periodoPrueba: '2026-09',
      bolsaPrueba: 3, bolsa: 150 }, 0, '2026-09');
...
describe('Aplicar un pago', () => {
  ...
  it('una bolsa suma 150 y no toca el período pagado ni el plan', () => {
    const r = aplicarPago({ modalidad: 'prepago', plan: 'base', periodoPagado: '2026-09', bolsa: 20 },
      { tipo: 'bolsa', cantidad: 2 }, '2026-09');
    expect(r).toMatchObject({ bolsa: 320, periodoPagado: '2026-09', plan: 'base' });
  });
...
describe('Recordatorios', () => {
  ...
  it('en demostración no se manda nada', () => {
    expect(recordatoriosDebidos({}, estadoEl({}, 0, '2026-09'), 'Demo', bo('2026-09-29 09:00'))).toEqual([]);
  });

  it('a mitad de mes, con el mes pagado, no se manda nada', () => {
    const r = recordatoriosDebidos(pagado, estadoEl(pagado, 0, '2026-09'), 'Salón Rosa', bo('2026-09-15 09:00'));
    expect(r).toEqual([]);
  });

  it('el primer aviso de renovación sale a 7 días del fin de mes; el segundo, a 2', () => {
    const a7 = recordatoriosDebidos(pagado, estadoEl(pagado, 0, '2026-09'), 'Salón Rosa', bo('2026-09-23 09:00'));
    expect(a7.map((r) => r.clave)).toEqual(['renovacion_2026-10_1']);
    expect(a7[0]).toMatchObject({ plantilla: PLANTILLAS.renovacion.nombre,
      parametros: ['Salón Rosa', 'Plan Base', '30/09/2026', '250'] });

    const a2 = recordatoriosDebidos(pagado, estadoEl(pagado, 0, '2026-09'), 'Salón Rosa', bo('2026-09-28 17:00'));
  ...
  it('un aviso ya enviado no se repite, aunque el flujo corra dos veces por día', () => {
    ...
    const r = recordatoriosDebidos(cuenta, estadoEl(cuenta, 0, '2026-09'), 'Salón Rosa', bo('2026-09-24 17:00'));
    expect(r).toEqual([]);
  });

  it('si el mes siguiente ya está pagado, no hay aviso de renovación', () => {
    const cuenta = { ...pagado, periodoPagado: '2026-10' };
    const r = recordatoriosDebidos(cuenta, estadoEl(cuenta, 0, '2026-09'), 'Salón Rosa', bo('2026-09-29 09:00'));
    expect(r).toEqual([]);
  });
  ...
  it('el mes de prueba también avisa que hay que elegir plan', () => {
    ...
    const r = recordatoriosDebidos(cuenta, estadoEl(cuenta, 0, '2026-09'), 'Salón Rosa', bo('2026-09-24 09:00'));
  ...
  it('cortado por falta de pago: UN aviso, con los mensajes perdidos', () => {
    ...
    const r = recordatoriosDebidos(cuenta, estadoEl(cuenta, 0, '2026-10'), 'Salón Rosa', bo('2026-10-01 09:00'));
    expect(r).toHaveLength(1);
    ...
    expect(recordatoriosDebidos(enviado, estadoEl(enviado, 0, '2026-10'), 'Salón Rosa', bo('2026-10-03 09:00'))).toEqual([]);
  });
  ...
  it('cortado por conversaciones: uno al cortar y otro a los dos días', () => {
    ...
    const estado = estadoEl(cuenta, 300, '2026-09');
    expect(estado.motivo).toBe('sin_conversaciones');

    const hoy = recordatoriosDebidos(cuenta, estado, 'Salón Rosa', bo('2026-09-20 17:00'));
    expect(hoy.map((r) => r.clave)).toEqual([`corte_conversaciones_${desde}_1`]);
    expect(hoy[0]?.parametros).toEqual(['Salón Rosa', 'Plan Base', '9', '50']);

    const enDos = recordatoriosDebidos(cuenta, estado, 'Salón Rosa', bo('2026-09-22 17:00'));
  ...
  it('un segundo corte en el mismo mes tiene claves nuevas: se avisa otra vez', () => {
    ...
    const r = recordatoriosDebidos(cuenta, estadoEl(cuenta, 300, '2026-09'), 'Salón Rosa', bo('2026-09-25 17:00'));
...
describe('Los textos que lee el negocio', () => {
  ...
  it('el resumen de cuenta dice lo que corresponde a cada estado', () => {
    const pagado = { modalidad: 'prepago', plan: 'base', periodoPagado: '2026-09', bolsa: 150 };
    expect(resumenDeCuenta(estadoDeServicio(pagado, 10, '2026-09'), 'Salón Rosa'))
      .toMatch(/Plan Base, pagado hasta el 30\/09\/2026\. Te quedan 440 conversaciones/);
    expect(resumenDeCuenta(estadoDeServicio(pagado, 0, '2026-10'), 'Salón Rosa')).toMatch(/DETENIDO.*no está pagado/);
    expect(resumenDeCuenta(estadoDeServicio({ ...pagado, bolsa: 0 }, 300, '2026-09'), 'Salón Rosa'))
      .toMatch(/DETENIDO.*agotaron/);
    expect(resumenDeCuenta(estadoDeServicio({}, 0, '2026-09'), 'Demo')).toMatch(/demostración/);
    for (const texto of [
      resumenDeCuenta(estadoDeServicio(pagado, 10, '2026-09'), 'X'),
      resumenDeCuenta(estadoDeServicio(pagado, 0, '2026-10'), 'X'),
