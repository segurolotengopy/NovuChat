...
case "$MARCADOR" in REEMPLAZAR_*) ;; *) echo "✗ Un marcador empieza con REEMPLAZAR_." >&2; exit 2 ;; esac
...
case "$VALOR" in *'|'*) echo "✗ El valor lleva «|» y rompería la tabla." >&2; exit 1 ;; esac

tmp="$(mktemp)"; trap 'rm -f "$tmp"' EXIT
awk -v m="$MARCADOR" -v v="$VALOR" -v n="$NOTA" '
...
cp -p "$LOCAL" "$LOCAL.respaldo" && cat "$tmp" > "$LOCAL"
echo "✓ $MARCADOR agregado a la tabla (respaldo en CONFIGURACION.local.md.respaldo)."
