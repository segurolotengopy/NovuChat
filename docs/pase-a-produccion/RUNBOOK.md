...
| El chat **cambia de número y de portafolio** (el comercio trae su portafolio; primer caso: el chat interno de NovuChat al portafolio de Silvana, 24/09/2026, `CLIENTES/NOVUCHAT/traspaso-tech-provider.md`) | El mismo camino B, **sin baja ni alta de número**: app y usuario de sistema **en el portafolio del comercio**, el viejo número se queda y la app vieja se desuscribe de su WABA (`verificar-meta.sh --desuscribir`). La plantilla se vuelve a pedir con el mismo texto (`listar-plantillas.sh --texto` → `crear-plantilla.sh --encabezado/--pie`). **Tech Provider todavía no:** AAB1 no puede crear usuarios de sistema ni emitir tokens (restricción de Meta, documentada en el otro proyecto de AAB1). El día que tenga un token que opere WABAs de clientes, la app compartida `AAB1-WA-Prod` es también la de otro producto (prohibición 5): su URL no se toca, el webhook va **a nivel de WABA** (`webhook-meta.sh --alta-waba`, `override_callback_uri`; `--ver-waba` lo muestra), y el flujo ya filtra por `phone_number_id` en `¿Es un mensaje?` |
