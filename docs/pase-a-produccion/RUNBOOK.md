...
| El chat **cambia de número y de portafolio**, operado por NovuChat como **Tech Provider** (la app de AAB1 sobre la WABA del comercio, compartida como socio) | El mismo camino B, **sin baja ni alta de número**: el viejo se queda y la app vieja se desuscribe de su WABA (`verificar-meta.sh --desuscribir`). Antes de dar de alta el webhook, `webhook-meta.sh --ver-meta`: **una app tiene una sola URL**, y la app Tech Provider no puede ser la de otro producto (prohibición 5). La plantilla se vuelve a pedir con el mismo texto (`listar-plantillas.sh --texto` → `crear-plantilla.sh --encabezado/--pie`). Primer caso: el chat interno de NovuChat, 24/09/2026 (`CLIENTES/NOVUCHAT/traspaso-tech-provider.md`) |
