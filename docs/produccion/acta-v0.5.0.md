...
Commit: **`742eaf7`** (`origin/main`, merge del PR #90) | Run CI en `main`: «CI/CD Node → Firebase» run **35179897465** sobre `742eaf7`, en verde; CodeQL (35179897240) y OpenSSF Scorecard (35179897255) también en verde sobre el mismo commit | Modo: **A**
...
| 2 | Run de CI completo en verde para el commit candidato | Cumple | Run **35179897465**: `preparar`, `calidad-documentacion`, `calidad`, `construir (staging/production)`, `seguridad-estatica/*` (Gitleaks, Semgrep, Trivy fs), `compuerta-pr` — todos `success`. `IaC (Checkov)` y `Snyk` en `skipped` (opcionales, sin secreto/config, no bloquean) |
| 3 | CodeQL y Scorecard activos | Cumple | Runs 35179897240 y 35179897255 sobre `742eaf7`, ambos `success` |
...
| 6 | `security-local.sh` reciente (≤ 24 h) | **No verificado** | No existe `.security-reports/` en este worktree (directorio gitignoreado, `grep .gitignore` línea 43). No se ejecutó aquí ni se encontró un informe con fecha del pase. Equivalente automatizado: el job `seguridad-estatica` (Semgrep, Trivy fs, Gitleaks) del run 35179897465, en verde |
