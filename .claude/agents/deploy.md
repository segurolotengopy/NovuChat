...
> Su ruta real esta en `CONFIGURACION.local.md` (ignorado por git): este
> repositorio es publico y no lleva rutas absolutas.
...
2. `${RUTA_ESTANDAR}/03-scripts/deploy.sh` (`--help` y encabezado: fases, códigos de salida) y `security-local.sh`.
3. `${RUTA_ESTANDAR}/00-gobernanza/02-flujo-git-y-versionado.md` y `03-ambientes-modos-y-aprobaciones.md` (qué ambiente se despliega desde dónde y quién aprueba).
4. `${RUTA_ESTANDAR}/01-seguridad/06-rollback-e-incidentes.md` antes de cualquier rollback.
5. `${RUTA_ESTANDAR}/01-seguridad/03-hardening-por-nube.md` para los comandos específicos del proveedor.
