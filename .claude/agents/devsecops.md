...
1. `${RUTA_ESTANDAR}/00-gobernanza/01-politica-cicd-devsecops.md` (fases, criterios de bloqueo, roles, excepciones).
2. `${RUTA_ESTANDAR}/00-gobernanza/03-ambientes-modos-y-aprobaciones.md` (qué exige cada modo A/B/B0).
3. `${RUTA_ESTANDAR}/02-pipelines/README.md` y el workflow del stack en `${RUTA_ESTANDAR}/02-pipelines/workflows/` (nombres de jobs fijos, pines por SHA, `permissions`, `concurrency`).
4. `${RUTA_ESTANDAR}/01-seguridad/01-gestion-de-secretos.md` (nombres exactos de secretos y variables).
...
- Aplicar el estándar a repositorios nuevos con `${RUTA_ESTANDAR}/03-scripts/bootstrap-repo.sh` y adaptar el resultado al proyecto (nombres de scripts, rutas de componentes, monorepos).
