...
1. Ruta local del estándar: `${RUTA_ESTANDAR}` (confírmela con `ls ${RUTA_ESTANDAR}/03-scripts/bootstrap-repo.sh`; si no existe, pregunte la ruta).
2. Lea `${RUTA_ESTANDAR}/00-gobernanza/01-politica-cicd-devsecops.md` (secciones 4 y 12: fases y mapeo v1 → v2) y `${RUTA_ESTANDAR}/02-pipelines/README.md`.
...
```bash
${RUTA_ESTANDAR}/03-scripts/bootstrap-repo.sh --stack <stack> --modo <A|B|B0> --ruta-estandar ${RUTA_ESTANDAR}
```
