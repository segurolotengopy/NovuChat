...
> **Solo para agregar.** Cada asiento se escribe una vez y no se corrige:
> si algo cambió, se agrega un asiento nuevo que lo diga. Es la mitad de
> `ESTADO.md` que no se puede derivar de git, GitHub, la nube ni n8n
> (`Analisis/41` §5.5 y §9.4): decisiones, hallazgos, lo que costó tiempo.
> Lo derivable lo imprime `scripts/estado-generado.sh`; el resumen de una
> pantalla es `ESTADO.md`.
>
> Los asientos de abajo son los de `ESTADO.md` hasta el 26/09/2026, movidos
> **sin cambiar una letra** y en el orden en que estaban (lo más nuevo arriba,
> como se escribía). Sin secretos ni identificadores.

## Cabecera de `ESTADO.md` al 26/09/2026, antes de partirlo (la cadena de «Última actualización»)
...
## 2026-09-26 (madrugada, 3) — F1 cerrada: ejes migrados y `v0.10.0` en producción

- **Migración de los ejes** (`admin/scripts/migrar-ejes.mjs`, con el «sí» de
  Andres), en el orden de la revisora: contados los tenants reales (**5**, no
  6), seco leído entero, aplicado, releído y un segundo seco con **0
  cambios**. Una auditoría `migrar_ejes` por tenant con el operador. Los
  demos pasan de plan `demostracion` a **Pro con modalidad demostración**:
  las reglas de F1 ya no reconocen `demostracion` como plan, por eso se migró
  **antes** de desplegar. Bellido y Platinum quedan sin modalidad explícita
  (se tratan como demostración; la fija su sesión). Bellido queda con 0
  cambios incluidos por Impulso: un cambio operado se rechaza salvo con
  `forzar`, auditado.
- **`v0.10.0`** creada por Andres sobre `4f7e091`; run del pipeline en verde
  (`desplegar-produccion` con su `--dry-run` previo y `post-despliegue`).
  Verificado después: 55 de 55 servicios con la última revisión lista; cero
  errores en los registros; las tres callables nuevas con `allUsers`, con
  `sa-functions` y contestando con **nuestro** código sin credenciales
  (`asignarEjes` y `registrarCambioOperado` 401 «Inicie sesión.»;
  `ejesDeCuenta` 400 por la forma del identificador, que valida antes de
  exigir sesión y antes de leer nada). Sin mensajes en `ingesta` desde el
  despliegue (00:40): el primero real ejercita el origen del anuncio.
- **S no llegó a la etiqueta** (el proyecto de staging no tiene facturación):
  se aplicó la alternativa acordada con la revisora.
- **Costo:** 0 mensajes por conversación; 1 etiqueta y 1 despliegue; 15
  escrituras en Firestore (5 cuentas, 5 fichas, 5 rutas) más 5 auditorías.
